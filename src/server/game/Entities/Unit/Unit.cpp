...
ProcEventInfo::ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget,
                             ProcFlags typeMask, ProcFlagsSpellType spellTypeMask,
...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, uint32 unmitigatedDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    else if (victim->IsCreature() && victim != attacker && damage >= health && victim->ToCreature()->HasStaticFlag(CREATURE_STATIC_FLAG_UNKILLABLE))
    {
        damage = health - 1;
    }
...
void Unit::InterruptSpell(CurrentSpellTypes spellType, bool withDelayed, bool withInstant)
{
    ...
    if ((withDelayed || spell->getState() != SPELL_STATE_LAUNCHED) &&
...
void Unit::_RemoveNoStackAurasDueToAura(Aura* aura, bool owned)
{
    ...
    if (owned)
        RemoveOwnedAuras([aura](Aura const* ownedAura) { return !aura->CanStackWith(ownedAura); }, AuraRemoveFlags::ByDefault);
    else
        RemoveAppliedAuras([aura](AuraApplication const* appliedAura) { return !aura->CanStackWith(appliedAura->GetBase()); }, AuraRemoveFlags::ByDefault);
}
...
void Unit::ProcSkillsAndReactives(bool isVictim, Unit* procTarget, ProcFlags typeMask, ProcFlagsHit hitMask, WeaponAttackType /*attType*/)
{
...
void Unit::TriggerAurasProcOnEvent(Unit* actionTarget, ProcFlags typeMaskActor, ProcFlags typeMaskActionTarget, ProcFlagsSpellType spellTypeMask,
...
{
    Unit::AuraEffectList const& swaps = GetAuraEffectsByType(SPELL_AURA_OVERRIDE_ACTIONBAR_SPELLS);
    ...
    for (AuraEffect const* auraEffect : swaps2)
    {
        if (uint32(auraEffect->GetMiscValue()) == spellInfo->Id || auraEffect->IsAffectingSpell(spellInfo))
        {
            if (SpellInfo const* newInfo = sSpellMgr->GetSpellInfo(auraEffect->GetAmount()))
