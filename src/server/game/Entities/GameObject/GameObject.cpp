...
bool GameObject::Create(ObjectGuid::LowType guidlow, uint32 name_id, Map* map, Position const& pos, QuaternionData const& rotation, uint32 animprogress, GOState go_state, uint32 artKit /*= 0*/, bool dynamic, ObjectGuid::LowType spawnid)
{
    ...
    switch (goinfo->type)
    {
        ...
        case GAMEOBJECT_TYPE_TRAP:
            ...
            if (SpellInfo const* trapSpell = sSpellMgr->GetSpellInfo(goinfo->trap.spell))
            {
                ...
                if (trapSpell->IsPositive())
                {
                    ...
                    {
                        SpellEffectInfo const& spellEffectInfo = trapSpell->Effects[i];
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (GetOwner())
                    {
                        ...
                        if (SpellInfo const* trapSpell = sSpellMgr->GetSpellInfo(goInfo->trap.spell))
                        {
                            ...
                            target = (worldObjectTarget && worldObjectTarget->IsUnit()) ? worldObjectTarget->ToUnit() : nullptr;
                        }
                        else
                        {
...
void GameObject::Use(Unit* user)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_SPELLCASTER:                   //22
        {
            ...
            spellId = m_goInfo->spellcaster.spell;
