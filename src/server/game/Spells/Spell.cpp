...
void Spell::SendCastResult(SpellCastResult result, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/) const
{
    ...
    SendCastResult(receiver, m_spellInfo, m_cast_count, result, m_customError, param1, param2);
}
        ...
        if (!targetGUID.IsEmpty())
        {
            ...
            if (ihit != std::end(m_UniqueTargetInfo))
            {
                ...
                if (Player* modOwner = unitCaster->GetSpellModOwner())
                    modOwner->ApplySpellMod(m_spellInfo, SpellModOp::PowerCostOnMiss, m_powerCost);
            }
        }
    }
...
SpellCastResult Spell::CheckRuneCost(uint32 runeCostID) const
{
    ...
    for (uint8 i = 0; i < AsUnderlyingType(RuneType::Death); ++i)
    {
        ...
        if (Player* modOwner = m_caster->GetSpellModOwner())
            modOwner->ApplySpellMod(m_spellInfo, SpellModOp::PowerCost0, runeCost[i], const_cast<Spell*>(this));
    }

    runeCost[AsUnderlyingType(RuneType::Death)] = MAX_RUNES; // calculated later
...
void Spell::TakeRunePower(SpellMissInfo hitInfo)
{
    ...
    // If the spell does not require any rune because of power cost spell mods, the Death Knight won't gain any Runic Power (Freezing Fog for example)
    if (!hasRuneCost && !runeCostData->NoRuneCost())
        return;

    // The attack refunds its runes when missed
    if (m_spellInfo->HasAttribute(SPELL_ATTR1_DISCOUNT_POWER_ON_MISS) && hitInfo == SPELL_MISS_MISS)
        return;
    ...
    // - Death rune, any kind
    runeCost[AsUnderlyingType(RuneType::Death)] = 0;                               // calculated later
    ...
    if (int32 runicPowerGain = runeCostData->RunicPower * sWorld->getRate(RATE_POWER_RUNICPOWER_INCOME))
    {
        ...
        if (Player* modOwner = m_caster->GetSpellModOwner())
            modOwner->ApplySpellMod(m_spellInfo, SpellModOp::PowerCost0, runicPowerGain, const_cast<Spell*>(this));
...
void Spell::TakeReagents()
{
    ...
    if ((castItemTemplate && castItemTemplate->GetFlags() & ITEM_FLAG_NO_REAGENT_COST) || m_spellInfo->HasAttribute(SPELL_ATTR6_DO_NOT_CONSUME_RESOURCES))
        return;
...
void Spell::HandleHolyPower(Player* caster)
{
    ...
    if (hit)
    {
        modOwner->ApplySpellMod(m_spellInfo, SpellModOp::PowerCost0, m_powerCost, this);
...
SpellCastResult Spell::CheckCast(bool strict, uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/, MountResult* mountResult /*= nullptr*/)
{
    ...
    if (m_spellInfo->HasAttribute(SPELL_ATTR7_DEBUG_SPELL) && m_caster->ToUnit() && !m_caster->ToUnit()->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_ALLOW_CHEAT_SPELLS))
    {
...
SpellCastResult Spell::CheckRange(bool strict) const
{
    ...
    if (target && target != m_caster)
    {
        ...
        if (minRange > 0.0f && m_caster->GetExactDistSq(target) < minRange && !target->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_ATTACKER_IGNORES_MINIMUM_RANGES))
...
SpellCastResult Spell::CheckItems(uint32* param1 /*= nullptr*/, uint32* param2 /*= nullptr*/) const
{
    ...
    bool needReagentCheck = [&]()
    {
        ...
        if (!(_triggeredCastFlags & TRIGGERED_IGNORE_REAGENT_COST) || (player && !player->CanNoReagentCast(m_spellInfo)))
            return false;
...
bool Spell::CheckEffectTarget(Unit const* target, uint32 eff, Position const* losPosition) const
{
    ...
    switch (m_spellInfo->Effects[eff].Effect)
    {
        case SPELL_EFFECT_RESURRECT_NEW:
            ...
            if (target != m_caster && !IsWithinLOS(m_caster, target, true, VMAP::ModelIgnoreFlags::M2))
            {
        ...
        case SPELL_EFFECT_SKIN_PLAYER_CORPSE:
        {
            if (!m_targets.GetCorpseTargetGUID())
            {
                if (IsWithinLOS(m_caster, target, true, VMAP::ModelIgnoreFlags::M2) && target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_SKINNABLE))
                    return true;
...
void Spell::DoEffectOnLaunchTarget(TargetInfo& targetInfo, float multiplier, uint8 effIndex)
{
    ...
    {
        bool isAoeTarget = m_spellInfo->Effects[effIndex].IsTargetingArea() || m_spellInfo->Effects[effIndex].IsAreaAuraEffect() || m_spellInfo->Effects[effIndex].IsEffect(SPELL_EFFECT_PERSISTENT_AREA_AURA);
